...
> 详细的架构流程图（Agent 执行循环、ReAct 推理、Skill Submit-Poll、RL 训练流程等）请参见 [docs/architecture.md](docs/architecture.md)。

### 四大基础设施层

| 层级 | 名称 | 说明 | 详细文档 |
|------|------|------|----------|
| Layer 1 | **Built-in Governance** | 内容审核、Token Bucket 速率限制、RBAC 访问控制、审计日志 | [docs/governance.md](docs/governance.md) |
| Layer 2 | **Memory & Context Management** | 多模态工作记忆、3 种压缩策略、ChromaDB 向量上下文 | [docs/memory-management.md](docs/memory-management.md) |
| Layer 3 | **Dual-Level Retrieval** | BM25 + Semantic + RRF 混合检索、CJK 分词、知识库增强 | [docs/dual-retrieval.md](docs/dual-retrieval.md) |
| Layer 4 | **Multi-Model Router** | 6 种路由策略、Circuit Breaker 熔断器、异步健康检查 | [docs/model-router.md](docs/model-router.md) |
...
| 方法 | 路径 | 描述 |
|------|------|------|
| GET | `/api/v1/context/retrieval/query` | 查询知识库 (BM25 + Semantic + RRF 融合) |
